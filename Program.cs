...
void Learn()
{
    try
    {
        ...
        learningWords = SelectRandomNElems(learningWords, lim);
        ...
        while (wordsCount != 0)
        {
            ...
            while (markedCnt != wordsCount)
            {
                ...
                if (ans == dict[randEl])
                {
                    ...
                    Console.WriteLine($"Good! [{stat[randEl].TrueAnswers} of {stat[randEl].Asked}]");
                    goodAns++;
                    wordsToDel.Add(randEl);
                }
                else
                {
                    Console.WriteLine($"Wrong! [{stat[randEl].TrueAnswers} of {stat[randEl].Asked}]");
...
List<string> SelectRandomNElems(List<string> learningWords, int lim)
{
    var randRange = learningWords.Count;
    ...
    while (i < lim)
    {
        ...
        resList.Add(learningWords[currNum]);
        visited.Add(currNum);
        ++i;
    }
